...
class ImageFolder_2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = tiff.imread(os.path.join(self.dir,img_file_name+'.tif')).transpose([2, 0, 1])
        if img is None:
        ...
        return img_file_name, img, mask, boundary
...
def image_transform(img):
    ...
    img = (img - np.min(img)) * (4096 - 0) / ((np.max(img) - np.min(img)) + 1e-12)
...
def image_transform(img):
    ...
    img = np.float32(img)
    # img = (img - img.min())/(img.max()-img.min())
    img = (img - np.min(img)) * (256 - 0) / ((np.max(img) - np.min(img)) + 1e-12)
