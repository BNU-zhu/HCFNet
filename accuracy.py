...
def evaluate(model, data_loader):#device
    ...
    with torch.no_grad():
        for iter, data in enumerate(tqdm(data_loader)):
            ...
            targets = targets.cuda()#to(device)
            outputs1,outputs2,outputs3 = model(inputs.float())
            #output_mask = outputs1.sigmoid().detach().cpu().numpy().squeeze()
            output_mask = outputs1.detach().cpu().numpy().squeeze()
            output_mask = np.argmax(output_mask, axis=0)
            output_boundary = outputs2.detach().cpu().numpy().squeeze()
            output_boundary = np.argmax(output_boundary, axis=0)
            ...
            res1 =output_mask*255
            res2 =output_boundary*255
            ...
            '''
    return acc_meter1.avg * 100, acc_meter2.avg * 100
